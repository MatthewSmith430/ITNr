...
mixing_matrix_igraph<-function(gs,attrname,type){
  H<-c("name",attrname)
  ...
  if (is.null(E(gs)$weight)==TRUE){
    igraph::E(gs)$weight<-rep(igraph::ecount(gs),1)
  }else{E(gs)$weight<-E(gs)$weight}

  EL<-igraph::get.data.frame(gs,what="edges")
  EL<-dplyr::select(EL,to,from,weight)
  EL2<-EL
